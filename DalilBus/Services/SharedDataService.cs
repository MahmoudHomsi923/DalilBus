...
namespace DalilBus.Services
{
    public class SharedDataService : INotifyPropertyChanged
    {
        ...
        {
            string filter = $"&startPlaceID=eq.{SelectedStartPlace?.Id}&destinationPlaceID=eq.{SelectedDestinationPlace?.Id}&depatureDate=eq.{selectedDate.ToString("yyyy-MM-dd")}&depatureTime=gte.{selectedTime.ToString(@"hh\:mm\:ss")}";
            string orderBy = "&order=depatureDate.asc,depatureTime.asc";
            TravelsList = await ApiClient.GetFromSubabaseAsync<List<Travel>>(ApiConfig.TravelsEndpoint+filter+orderBy) ?? new List<Travel>();
        }
